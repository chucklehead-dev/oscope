...
The TUI's screenshots come from the binary itself. `oscope-tui --snapshot out.html --keys "..."`
replays a key script against a headless ratatui backend and writes the final frame as HTML.
Playwright then screenshots and asserts on it like any page, so no terminal emulator or
recording tool is involved.
