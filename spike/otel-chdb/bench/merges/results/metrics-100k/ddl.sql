CREATE TABLE mrg_metrics_100k.otel_metrics_number_points
(
...
SETTINGS non_replicated_deduplication_window = 1000, index_granularity = 8192, old_parts_lifetime = 30;

CREATE TABLE mrg_metrics_100k.otel_metrics_histogram_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
    series_id UInt64 CODEC(ZSTD(1)),
...
CREATE TABLE mrg_metrics_100k.otel_metrics_exponential_histogram_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
    series_id UInt64 CODEC(ZSTD(1)),
...
CREATE TABLE mrg_metrics_100k.otel_metrics_summary_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
    series_id UInt64 CODEC(ZSTD(1)),
...
CREATE TABLE mrg_metrics_100k.otel_metrics_series
(
    series_id UInt64,
...
SETTINGS index_granularity = 1024, allow_dimensions_outside_sorting_key = 1, old_parts_lifetime = 30;
