...
MERGES = os.path.join(SPIKE, "bench", "merges")  # the original DDL (sql/)
...
def ddl(name, a):
    ...
    if name in ("traces", "logs"):
        s = open(os.path.join(MERGES, "sql", f"otel_{name}.sql")).read()
...
def stmt(name, seq, size, recv_ns, a):
    ...
    if name.startswith("A_"):
        ...
        n = 2 if t == "sum" else (6 if big else 1)
        if t == "sum" and not big:
            raise SystemExit("A_sum is 52k rows per object; run it at --rows 100k (two objects)")
...
def ch_pid():
    for l in open(os.path.join(os.environ.get("MERGESRV", "/tmp/claude-0/-home-user/db86342c-d57d-54b7-95b1-90f220828b73/scratchpad/clean/chpriv"),
                               "data", "status")):
        if l.startswith("PID"):
...
def sample(a, pid, t0):
    ...
    return {"t": round(time.time() - t0, 2), "wall": time.time(), "tables": levels,
            ...
            "procstat": [int(x) for x in open("/proc/stat").readline().split()[1:9]]}
...
def main():
    ...
    final = ch(f"""SELECT table, count(), sum(rows), sum(bytes_on_disk), sum(data_uncompressed_bytes), toString(groupArray((level, rows))), sum(data_compressed_bytes)
