...
def a_stmt(t, seq, tcols):
    n_obj = {"sum": 2, "gauge": 2, "histogram": 6, "exponential_histogram": 6, "summary": 6}[t]
...
def main():
    ...
    for label, table, stmts, create in jobs:
        ...
        if stmts is None:  # layout A: needs the table's columns first
            ...
            n = {"sum": 60, "gauge": 60, "histogram": 20, "exponential_histogram": 20, "summary": 20}[t]  # 120 rounds = 5 pool cycles
            stmts = [a_stmt(t, s, tcols) for s in range(n)]
