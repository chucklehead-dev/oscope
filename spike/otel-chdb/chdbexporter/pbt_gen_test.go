...
type pdataGen struct {
	...
	// minTS and maxTS bound timestamps; maxTS 0 means any uint64.
	minTS, maxTS uint64
}
...
func (g pdataGen) ts(tc hegel.TestCase) uint64 {
	...
	special := []uint64{g.minTS, max}
	for _, v := range []uint64{0, 1, 1_000_000_000, 1e17, 1_790_000_000_123_456_789, math.MaxInt64, math.MaxInt64 + 1} {
		if v >= g.minTS && v <= max {
			special = append(special, v)
		}
	}
	return hegel.Draw(tc, hegel.OneOf(hegel.Integers[uint64](g.minTS, max), hegel.SampledFrom(special)))
}
