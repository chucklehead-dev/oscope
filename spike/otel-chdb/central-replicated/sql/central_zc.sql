...
ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{shard}/central/otel_metrics_series', '{replica}')
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/central/otel_metrics_number_points', '{replica}')
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/central/otel_metrics_histogram_points', '{replica}')
PARTITION BY toDate(received_at)
ORDER BY (MetricName, ServiceName, series_id, TimeUnix)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, index_granularity = 8192;
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/central/otel_metrics_exponential_histogram_points', '{replica}')
PARTITION BY toDate(received_at)
ORDER BY (MetricName, ServiceName, series_id, TimeUnix)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, index_granularity = 8192;
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/central/otel_metrics_summary_points', '{replica}')
PARTITION BY toDate(received_at)
ORDER BY (MetricName, ServiceName, series_id, TimeUnix)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, index_granularity = 8192;
