CREATE DATABASE IF NOT EXISTS rsoak_rsoak1790373757;
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_traces (Timestamp DateTime64(9), TraceId String, SpanId String, ParentSpanId String, TraceState String,
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_traces', '{replica}') PARTITION BY toDate(received_at) ORDER BY (ServiceName, SpanName, toDateTime(Timestamp))
...
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_logs (Timestamp DateTime64(9), TraceId String, SpanId String, TraceFlags UInt8, SeverityText LowCardinality(String),
  ...
  PROJECTION by_content (SELECT content_key, count() GROUP BY content_key))
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_logs', '{replica}') PARTITION BY toDate(received_at) ORDER BY (ServiceName, Timestamp)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, old_parts_lifetime = 30;
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_metrics_series
(
    series_id UInt64,
...
ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_metrics_series', '{replica}')
...
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_metrics_number_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_metrics_number_points', '{replica}')
...
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_metrics_histogram_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
    series_id UInt64 CODEC(ZSTD(1)),
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_metrics_histogram_points', '{replica}')
PARTITION BY toDate(received_at)
ORDER BY (MetricName, ServiceName, series_id, TimeUnix)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, old_parts_lifetime = 30, index_granularity = 8192;
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_metrics_exponential_histogram_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
    series_id UInt64 CODEC(ZSTD(1)),
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_metrics_exponential_histogram_points', '{replica}')
PARTITION BY toDate(received_at)
ORDER BY (MetricName, ServiceName, series_id, TimeUnix)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, old_parts_lifetime = 30, index_granularity = 8192;
CREATE TABLE IF NOT EXISTS rsoak_rsoak1790373757.otel_metrics_summary_points
(
    MetricName LowCardinality(String) CODEC(ZSTD(1)),
    ServiceName LowCardinality(String) CODEC(ZSTD(1)),
    series_id UInt64 CODEC(ZSTD(1)),
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/rsoak_rsoak1790373757/otel_metrics_summary_points', '{replica}')
PARTITION BY toDate(received_at)
ORDER BY (MetricName, ServiceName, series_id, TimeUnix)
TTL toDateTime(received_at) + INTERVAL 3 MINUTE TO VOLUME 'cold', toDateTime(received_at) + INTERVAL 1 DAY DELETE
SETTINGS storage_policy = 'tiered_zc', allow_remote_fs_zero_copy_replication = 1, old_parts_lifetime = 30, index_granularity = 8192;
